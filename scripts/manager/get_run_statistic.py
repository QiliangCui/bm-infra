...
def get_run_statistic(_):
    """Groups and counts ProcessedStatus with timing statistics."""
    ...
    # Query to get the absolute total cases from the 'Cases' table
    total_cases_query = "SELECT COUNT(*) FROM Cases WHERE ID = @id"
    
    # Query to group by status, count occurrences, and sum TotalTime (microseconds)
    status_query = (

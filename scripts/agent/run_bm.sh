...
for var in MODEL DATASET NUM_PROMPTS INPUT_LEN OUTPUT_LEN EXPECTED_ETEL TENSOR_PARALLEL_SIZE MAX_NUM_SEQS MAX_NUM_BATCHED_TOKENS MAX_MODEL_LEN PREFIX_LEN; do
  if [ -n "${!var:-}" ]; then
    val="${!var}"
    val="${val#\'}"
    val="${val%\'}"

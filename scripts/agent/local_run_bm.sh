...
if ! $CONDA env list | grep -Fq "$ENV_NAME"; then
  ...
  if [[ -n "$TPU_INFERENCE_HASH" ]]; then
    ...
    # $CONDA run -n "$ENV_NAME" bash -c "pip install -U --pre jax jaxlib libtpu requests -i https://us-python.pkg.dev/ml-oss-artifacts-published/jax/simple/ -f https://storage.googleapis.com/jax-releases/libtpu_releases.html"
    $CONDA run -n "$ENV_NAME" bash -c "pip install --pre jax==0.8.2.dev20251201 --index-url https://us-python.pkg.dev/ml-oss-artifacts-published/jax/simple/ --find-links https://storage.googleapis.com/jax-releases/libtpu_releases.html"
    $CONDA run -n "$ENV_NAME" bash -c "pip install --pre jaxlib==0.8.2.dev20251201 --index-url https://us-python.pkg.dev/ml-oss-artifacts-published/jax/simple/ --find-links https://storage.googleapis.com/jax-releases/libtpu_releases.html"
    $CONDA run -n "$ENV_NAME" bash -c "pip install numba"
    $CONDA run -n "$ENV_NAME" bash -c "mkdir -p ../shared-wheels && gsutil cp gs://libtpu-tpu7x-releases/wheels/libtpu/libtpu-0.0.31.dev20251201+tpu7x-cp312-cp312-manylinux_2_31_x86_64.whl ../shared-wheels/"
    $CONDA run -n "$ENV_NAME" bash -c "pip install ../shared-wheels/libtpu-0.0.31.dev20251201+tpu7x-cp312-cp312-manylinux_2_31_x86_64.whl"

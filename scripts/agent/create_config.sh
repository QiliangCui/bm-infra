...
JSON_OUTPUT=$(gcloud spanner databases execute-sql "$GCP_DATABASE_ID" \
    ...
    --format="json" \
    --sql="SELECT RecordId, Model, CodeHash, MaxNumSeqs, MaxNumBatchedTokens, TensorParallelSize, MaxModelLen, Dataset, InputLen, OutputLen, NumPrompts, PrefixLen, ExpectedETEL, ExtraEnvs, AdditionalConfig, Device, ExtraArgs FROM RunRecord WHERE RecordId = '$RECORD_ID';")
